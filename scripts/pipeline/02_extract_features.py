...
LABELS      = os.path.join(_ROOT, "data", "labels.csv")
OUTPUT_CSV  = os.path.join(_ROOT, "data", "features.csv")
TARGET_SR   = 16_000
N_MFCC      = 40
...
def extract_textual(audio: np.ndarray, language: str = "") -> dict:
    from collections import Counter

    lang = language.lower().strip()
    if lang == "sinhala":
        pipe     = get_sinhala_asr()
        # Whisper takes 30s of audio at a time. Beyond that HF requires
        # long-form decoding, which must be opted into explicitly. No
        # simulated call exceeds 26.1s, so this branch never fires on the
        # training corpus and its transcripts are unchanged.
        if len(audio) > 30 * TARGET_SR:
            result = pipe(audio.astype(np.float32), return_timestamps=True)
        else:
    ...
    # Context-aware features (10 values: 5 Sinhala + 5 Tamil; zeros for other langs)
    ctx = extract_context_features(raw_text, language)
    feats.update(ctx)
...
def main():
    ...
    for _, row in df_labels.iterrows():
        ...
        textual  = extract_textual(y, language=row.get("language", ""))
